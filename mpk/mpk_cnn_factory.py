...
from .flax.mpk_cnn import MultipoleConv
...
class MultipoleCNNFactory:
    ...
    def __init__(self, kernel_shape: Sequence[int] = None, polynomial_degrees: Sequence[int] = None,
        ...
        assert np.unique(self.kernel_shape).size == 1, 'Only kernels with the same size in each dimension are supported'
        assert np.unique(self.kernel_shape)[0] % 2 == 1, 'Only kernels with uneven kernel size are supported'

        if self.kernel_shape is None:
            self.kernel_shape = np.array([3, 3])
    ...
    def get_indices(self):
        ...
        indices = np.zeros((0, self.dimension + 2), dtype=int)
        weight_index = np.zeros(0, dtype=int)
        filter_counter = 0
        weight_counter = 0
        for i in range(len(self.polynomial_degrees)):
            ...
            for m in m_range:
                ...
                unique_distances = np.unique(modified_distances)
                if l != 0 and unique_distances[0] == 0:
                    ...
                    unique_distances = np.delete(unique_distances, 0)
                for dist in unique_distances:
                    temp_indices = np.argwhere(dist == modified_distances)
                    ...
                    for in_filt in range(self.num_input_filters):
                        for out_filt in range(self.output_filters[i]):
                            ...
                            # correct filter
                            filter_index = np.array([in_filt, filter_counter + out_filt])
                            extra_indices = np.tile(filter_index, np.array([num_elements, 1]))
                            final_indices = np.append(temp_indices, extra_indices, axis=1)
                            # TOASK: in general, is this in-place operation okay?
                            indices = np.concatenate((indices, final_indices))
                            weight_index = np.concatenate((weight_index, np.tile(weight_counter, [num_elements])))
                            weight_counter += 1
    ...
    def get_angles(self, grid, distance):
        if self.dimension == 1:
            ...
            return {"theta": np.arctan(np.abs(grid[1] / grid[0]))}
        elif self.dimension == 3:
            return {"theta": np.arccos(grid[2] / distance),
                    "phi": np.arctan(grid[1] / grid[0])}

    def get_symmetries(self, l, m, angles):
        if self.dimension == 1:
            return np.arange(self.kernel_shape[0])
        elif self.dimension == 2:
            return np.exp(complex(0., 1.) * angles["theta"] * l).real
        elif self.dimension == 3:
            theta = angles["theta"].flatten()
            phi = angles["phi"].flatten()
            res = scipy.special.sph_harm(np.full(self.kernel_size, m), np.full(self.kernel_size, l),
                                             np.array(theta), np.array(phi)).real
            return res.reshape(self.kernel_shape)

    def get_kernels(self):
        global_kernel_shape = np.append(self.kernel_shape,
                                         np.array([self.num_input_filters, self.num_output_filters]))

        kernels = np.zeros(np.append(self.num_params, global_kernel_shape))

        for i, weight_index in enumerate(self.weight_indices):
            kernels[weight_index][tuple(self.indices[i])] = 1

        # TODO: support multiple input filters
        return kernels.squeeze((self.dimension + 1,))

    def build_flax_cnn_model(self, pad_size=None,
                             num_input_filters=1,
                             backend="lax",
                             strides=None):

        if pad_size is None:
            pad_size = np.ceil((self.kernel_shape[0] - 1) / 2).astype(int)
